...
function saveFrontendFiles(galleryFactory: string, testNft: string) {
    ...
    fs.writeFileSync(
        ...
        JSON.stringify(
            { GalleryFactory: galleryFactory, TestNft: testNft },
            undefined,
            2
        )
    );
    ...
    fs.writeFileSync(
        contractsDir + '/Gallery.json',
        JSON.stringify(TokenArtifact, null, 2)
    );
    fs.writeFileSync(
        contractsDir + '/GalleryFactory.json',
        JSON.stringify(GalleryFactory, null, 2)
    );
    fs.writeFileSync(
        contractsDir + '/TestNFT.json',
        JSON.stringify(TestNFT, null, 2)
    );
}
